...
//        그래프에서 꺾이는 지점을 경계로 5개의 구역으로 나눠보면 각각의 구간 넓이는 10.5, 12, 6, 3, 1.5 입니다.
        ...
        System.out.println(Arrays.toString(getY(k)));
    ...
    public static double[] solution(int k, int[][] ranges) {
        ...
        for(int i=0; i<totalSize; i++){
            ...
            for(int j=0; j<ySize-1; j++) {
                int upLine = ranges[i][0] + y[j];
                int downLine = ranges[i][1] + y[j+1];
                y[j+1] = downLine;

                System.out.println(j + "/up : "+ upLine + "/down : " + downLine);
                answer[i] += (double) (upLine + downLine) / 2;
    ...
    public static int[] getY(int k){
        ...
        return Arrays.stream(resultList.toArray(new Integer[size]))

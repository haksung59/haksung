...
<link rel="stylesheet" type="text/css" href="<c:url value="/css/admin1/resvTotal.css"/>" />
<style>
td.td-page a{
...
<jsp:include page="Desktop/GitRepo/haksung/green academy/teamProj/src/main/webapp/views/base/adminDrop.jsp" />
	
	<div id="u-sch">
	<h2 id="title" align=center>예약조회</h2>	
	
	<form id="form1" name="searchForm" method="get">
		<p>
		    <input type="text" id="datepicker1" name="firdate" autocomplete="off" placeholder = "시작 날짜"  value="${indate }" > <b>~</b>
		    <input type="text" id="datepicker2" name="secdate" autocomplete="off" placeholder = "마지막 날짜" value=${outdate }>
		</p>    
		
		<p id ="id"><input type="text"  placeholder="아이디나 이름으로 검색"  name="name" value=${name }>
...
%>			
	<tr>
		<td id="td-none" colspan="11" align="center">예약 건이 없습니다.<br><br>날짜를 선택해주세요.</td>
	</tr>
<%
	}else{ 
%> 			
...
%>	
		...
		<tr align="center">
			...
			<tr>
				...
				if(start!=1){
					%>
						<a href="/admin1/reservation/total?pageIndex=<%=start-1 %>&&firdate=${indate }&&secdate=${outdate }&&name=${name}">◀</a>
					<% 
					}
							int pageBlock = 5;
					...
					%>
									<a href="/admin1/reservation/total?pageIndex=<%=i%>&&firdate=${indate }&&secdate=${outdate }&&name=${name}"><%=(i)%></a>
					<%				
								}
							}
					%>		
					<%
							if(start+4 < index){
					%>
							<a href="/admin1/reservation/total?pageIndex=<%=start+5 %>&&firdate=${indate }&&secdate=${outdate }&&name=${name}">▶</a>
					<%
							}
					%>
				</td>
